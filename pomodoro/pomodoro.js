...
sesIncDec.forEach(incDecBtn => incDecBtn.addEventListener("click", function(){
    if(!isRunning) sessionTime.textContent = changeTime(parseInt(sessionTime.textContent), parseInt(this.value));
}));
brIncDec.forEach(incDecBtn => incDecBtn.addEventListener("click", function(){
    if(!isRunning) breakTime.textContent = changeTime(parseInt(breakTime.textContent), parseInt(this.value));
}));
...
function displaySecs(secs) {
    ...
    const displayStr = `${mins < 10 ? '0' : ''}${mins}:${remainderSecs < 10 ? '0' : ''}${remainderSecs}`;
