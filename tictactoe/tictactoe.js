// For this project, I followed along with freeCodeCamp's "Unbeatable AI w/ Minimax Algorithm" tutorial.
//      link: https://www.youtube.com/watch?v=P2TcQ3h0ipQ
// Original minimax article: https://medium.freecodecamp.org/how-to-make-your-tic-tac-toe-game-unbeatable-by-using-the-minimax-algorithm-9d690bad4b37?gi=9ef446b8c31b
...
playerSelect.forEach(player =>
  player.addEventListener("click", selectPlayer, false)
);
...
function resetGame() {
  ...
  cellIcons.forEach(cellIcon =>
    cellIcon.classList.remove("far", "fa-circle", "fas", "fa-times")
  );

  playerSelect.forEach(player =>
    player.classList.add("ttt__overlay__select--visible")
  );
...
function selectPlayer(player) {
  ...
  playerSelect.forEach(player =>
    player.classList.remove("ttt__overlay__select--visible")
  );
}
...
function minimax(newBoard, player) {
  ...
  var bestMove;
  if (player === com) {
    bestMove = moves.reduce((acc, nxt) => (nxt.score > acc.score ? nxt : acc), {
      score: -10000
    });
    // var bestScore = -10000;
